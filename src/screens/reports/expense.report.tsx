...
export const ExpenseReport = () => {
  ...
  if (loading) {
    return <ReportsLayout title={t('titles.expense')} subtitle={subtitle}><div className="py-12 text-center text-muted">{t('loading.expense')}</div></ReportsLayout>;
  }
  if (error) {
    ...
    <ReportsLayout title={t('titles.expense')} subtitle={subtitle}>
      <div className="space-y-4">
        <div className="border rounded-lg p-4 bg-surface">
          <div className="text-sm text-muted">{t('labels.expenseRows')}</div>
          <div className="text-xl font-semibold">{formatNumber(rows.length)}</div>
          <div className="text-sm text-muted mt-2">Total expenses</div>
        ...
        <div className="overflow-hidden rounded-lg border border-border">
          <table className="min-w-full divide-y divide-neutral-200">
            ...
            <tr>
              <th className="py-3 pl-6 pr-3 text-left text-sm font-semibold text-foreground">{t('metrics.dateWindow')}</th>
              <th className="py-3 px-3 text-left text-sm font-semibold text-foreground">{t('columns.description')}</th>
              <th className="py-3 px-3 text-left text-sm font-semibold text-foreground">{t('columns.category')}</th>
              <th className="py-3 pr-6 text-right text-sm font-semibold text-foreground">{t('columns.amount')}</th>
            </tr>
            </thead>
            <tbody className="divide-y divide-neutral-100 bg-surface-elevated">
            {rows.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-6 text-center text-sm text-muted">No expenses found for selected range.</td>
              </tr>
            ) : rows.map((row, index) => (
              <tr key={`${row.closingId}-${index}`}>
                ...
                <td className="py-3 px-3 text-sm text-foreground">{row.description}</td>
                <td className="py-3 px-3 text-sm text-foreground">{row.category}</td>
                <td className="py-3 pr-6 text-right text-sm font-semibold text-foreground">{withCurrency(row.amount)}</td>
              </tr>
            ))}
            </tbody>
          </table>
