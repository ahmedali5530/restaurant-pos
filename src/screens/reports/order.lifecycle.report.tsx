...
type TimelineType = "start" | "addition" | "deletion" | "kitchen_complete" | "payment";
...
const normalizeOrderSuffix = (value: string) => {
  if (!value) return "";
  if (value.includes(":")) {
...
export const OrderLifecycleReport = () => {
  ...
  const orderSuffix = useMemo(() => normalizeOrderSuffix(filters.orderIdParam), [filters.orderIdParam]);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [orderRows] = await queryRef.current(
          `
            SELECT * FROM ONLY type::record('${Tables.orders}', $orderSuffix)
  ...
  const events = useMemo(() => {
    ...
    if (state.order.completed_at || (state.order.payments || []).length > 0) {
      ...
      timeline.push({
        ...
        type: "payment",
        timestamp: state.order.completed_at || state.order.created_at,
        title: "Order payment",
  ...
  const iconByType = {
    start: faPlayCircle,
    addition: faPlusCircle,
    deletion: faTrash,
    kitchen_complete: faUtensils,
    payment: faMoneyBillWave,
  };

  if (loading) {
    return <ReportsLayout title="Order lifecycle report" subtitle={subtitle}><div className="py-12 text-center text-neutral-500">Loading order lifecycle report...</div></ReportsLayout>;
  }
  if (error) {
    return <ReportsLayout title="Order lifecycle report" subtitle={subtitle}><div className="py-12 text-center text-red-600">Failed to load report: {error}</div></ReportsLayout>;
  }
  if (!state.order) {
    return <ReportsLayout title="Order lifecycle report" subtitle={subtitle}><div className="py-12 text-center text-neutral-500">No order found for provided ID.</div></ReportsLayout>;
  }

  return (
    <ReportsLayout title="Order lifecycle report" subtitle={subtitle}>
